...
execute store result score 通报部队 new run team list circular
execute store result score 任務数量 new run scoreboard players get 任務数 time
...
execute unless score 偵測開場遊戲 temp = 開場遊戲 temp if score 開場遊戲 temp matches 2 run tellraw @a ["",{"text":"[系統]","bold":true,"color":"gold"},"開場遊戲：",{"text":"掷骰子","color":"red"}]
execute unless score 偵測開場遊戲 temp = 開場遊戲 temp if score 開場遊戲 temp matches 3 run tellraw @a ["",{"text":"[系統]","bold":true,"color":"gold"},"開場遊戲：",{"text":"体内计时器","color":"red"}]
