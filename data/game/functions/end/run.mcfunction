...
execute if entity @a[team=run,tag=m18] run tellraw @a ["","背叛者成功活到游戏最后 额外获得",{"score":{"name":"額外","objective":"money"},"bold":true,"color":"gold"},{"text":"元","bold":true,"color":"gold"}]
...
title @a title ["",{"text":"游戏结束","bold":true,"color":"gold"}]
title @a subtitle ["",{"text":"时间结束","bold":true,"color":"gold"}]
